...
async def dm_upload_data(user, bot):
    ...
    embed=discord.Embed(title=f"PROFILE Picture Upload`",  description="*Default* `Timeout: 1mins` ", color=discord.Color.blue())
    #embed.set_image(url=(author.avatar_url))
    embed.set_thumbnail(url=user.avatar)
    embed.add_field(name="picture", value=f"Send a Picture you would like to display on your profile", inline=True)
