...
async def handle_upload_data(user, interaction, bot):
    selfie_verf = discord.utils.get(
        interaction.guild.roles, id=bot_configs.role("selfie_verf")
    )
    ...
    if selfie_verf in interaction.user.roles:
        ...
        category = discord.utils.get(
            interaction.guild.categories, id=bot_configs.channel("profile_category")
        )
        ...
        if user_in_chn:
            await interaction.response.send_message(
                content="You Have a Pending Profile Upload Channel",
            )
            ...
            button = Button(
                label="Go to Channel",
                url=f"https://discord.com/channels/{interaction.guild.id}/{channel_created.id}",
            ...
            await interaction.response.send_message(
                content="Selfie Upload channel created, click the button below to get to the channel!",
                view=view,
                ephemeral=True,
            )
            ...
            embed_first.set_author(
                name=f"{interaction.guild.name} Upload Selfie",
                icon_url=f"{interaction.guild.icon.url}",
            )
            embed_first.add_field(
                name="First picture",
                value=f"Upload a Selfie you which to displayed on your profile \n\n  **UPLOADED FILE SHOULD ONLY BE PICTURES**",
                inline=True,
            )

            embed_wanning = discord.Embed(color=discord.Color.blue())
            embed_wanning.set_author(
                name=f"{interaction.guild.name}",
                icon_url=f"{interaction.guild.icon.url}",
            )
            embed_wanning.add_field(
                name="------------------------",
