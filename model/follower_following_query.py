...
class Following_follower():
    ...
    async def follow_user(self, user_id, follower_id):
        ...
        try:
            self.cur.execute(f"INSERT INTO relationship VALUES (NULL, ?, ?)", (user_id, follower_id))
    ...
    def unfollower_user(self, user_id, follower_id):
        ...
        try:
            self.cur.execute(f"DELETE FROM relationship WHERE user_id = {user_id} AND follower_id = {follower_id}")
    ...
    def total_followers_count(self, user_id):
        ...
        try:
            count = self.cur.execute(f"SELECT COUNT(*) FROM relationship WHERE user_id = {user_id}")
    ...
    def total_following_count(self, user_id):
        try:
            count = self.cur.execute(f"SELECT COUNT(*) FROM relationship WHERE follower_id = {user_id};")
            return count.fetchone()[0]
    ...
    def following_list(self, user_id):
        ...
        try:
            ...
            self.cur.execute(f"SELECT * FROM relationship WHERE follower_id = {user_id};")
    ...
    def user_ff_bool(self, user_id, follower_id):
        ...
        try:
            self.cur.execute(f"SELECT * FROM relationship WHERE user_id={user_id} AND follower_id={follower_id}")
    ...
    def user_following_check(self, user_id, following_id):
        ...
        try:
            self.cur.execute(f"SELECT * FROM relationship WHERE user_id={user_id} AND follower_id={following_id}")
